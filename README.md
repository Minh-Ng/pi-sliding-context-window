...
- Mask every source message still visible in the provider window from explicit search and gather results. Settled turns remain immediately durable without allowing the model to retrieve its current context back as historical evidence.
...
Semantic snapshots keep only compact binary vector-label, document, and byte-range references. Result text and provenance are read from canonical RocksDB records only for ANN candidates that survive scope and score checks. A legacy `metadata.json` snapshot migrates to `metadata.bin` on first load without re-embedding.
...
Maintenance also performs a bounded, crash-resumable posting-storage cutover: obsolete reader-inaccessible index namespaces, compact BM25 session and folded exact locators, binary canonical posting blocks, paged verification of ordinal-tombstone authority, and cleanup-only reverse-reference retirement after a 24-hour query-cutover grace. Mixed legacy and compact values remain readable by the current reader throughout; the grace is not an older-binary downgrade guarantee. Use `context-window-index-maintenance inventory --store PATH` or `gc --store PATH` for a read-only namespace report; destructive offline GC requires `gc --store PATH --apply --offline`. See [posting-storage.md](./docs/rocksdb-archive/posting-storage.md) for invariants, measurements, and deferred designs.
