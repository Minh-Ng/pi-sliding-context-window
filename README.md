...
The package declares `@earendil-works/pi-coding-agent >=0.80.6` as its compatibility floor. The integration is tested against Pi 0.80.10. Transparent rotation specifically requires the host's pre-provider `context` message-transform hook, and threshold-compaction suppression requires cancellable `session_before_compact`; hosts without equivalents can use the MCP archive but not the full window policy.
