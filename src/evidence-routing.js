...
export const EVIDENCE_ROUTING_GUIDELINES = Object.freeze([
  ...
  "Before you quote or assert a specific earlier detail (a number, name, exact phrasing, or committed decision) not visible in the current context, run context_window_search for its anchor rather than reconstructing it from memory. A continuity marker only confirms that eligible earlier discussion matched the quoted anchor from the current user message; if prior project-specific meaning is material, search that exact anchor with scope=auto (or the marker's project scope) before answering, and do not treat the marker itself as a recovered fact, decision, definition, or current-state claim.",
