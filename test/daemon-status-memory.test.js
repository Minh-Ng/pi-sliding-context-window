...
test("metadata-heavy status and exact count stay below the daemon RSS gate", (t) => {
  const directory = mkdtempSync(join(tmpdir(), "context-window-status-memory-"));
  const storePath = join(directory, "archive.rocks");
  t.after(() => rmSync(directory, { recursive: true, force: true }));

  const setup = runFixture(["setup", storePath, String(DOCUMENT_COUNT)]);
  assert.equal(setup.status, 0, setup.stderr || setup.stdout);

  const measured = runFixture(["measure", storePath, String(DOCUMENT_COUNT)]);
  assert.equal(measured.status, 0, measured.stderr || measured.stdout);
  const result = JSON.parse(measured.stdout);
  assert.equal(result.statusApproximate, true);
  assert.equal(result.statusDocuments > 0, true);
  assert.equal(result.statusDocuments < DOCUMENT_COUNT, true);
  assert.equal(result.exactCount, DOCUMENT_COUNT);
  assert.equal(
